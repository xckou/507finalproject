...
API_KEY= "-0MtYjViQrfSIhSCR0BEcgITakr16iWh2RcfIIT2KKWB1s2C3G8xSP6N16FgLLIXySWkwKTM5FvFO-QMp43D2tYq9A2rJy8QS0yu-I1ED6b4wrP5bQTScR6KvSnOWnYx" 
...
CACHE_FNAME = 'yelp_resturant.json'
...
def populate_tournament_db():
    ...
    for a in range(len(json_dic)):
        Name = json_dic["{}".format(a)]["name"]
        Attending = json_dic["{}".format(a)]["attending_count"]
        Catagory = json_dic["{}".format(a)]["category"]
        Cost = json_dic["{}".format(a)]["cost"]
        Latitude = json_dic["{}".format(a)]["latitude"]
        Longitude = json_dic["{}".format(a)]["longitude"]
...
def plot_yelp_event():
    ...
    layout = dict(
        title = 'Seattle Events<br>(Hover for names)',
        geo = dict(
            scope='usa',
            projection=dict( type='albers usa' ),
            showland = True,
